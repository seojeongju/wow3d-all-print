        ...
        const run = async () => {
            try {
                ...
                if (user?.id && user?.email) {
                    ...
                    showToast.success(
                        '로그인 성공',
                        kakaoOAuth ? `${user.name}님, 카카오 계정으로 로그인했습니다.` : `${user.name}님, 환영합니다.`
                    )
                ...
                >
                    ...
                    <form onSubmit={isLogin ? handleLogin : handleSignup} className="space-y-5">
                        ...
                        <div className="grid gap-3">
                            ...
                            <a
                                href={`/api/auth/kakao${returnTo ? `?return=${encodeURIComponent(returnTo)}` : ''}`}
                                className="flex items-center justify-center gap-3 w-full h-14 rounded-xl bg-[#FEE500] hover:bg-[#ffe033] text-[#191919] font-semibold border border-[#e6cf00]/80 transition-all duration-200"
                            >
