...
type MediaItem = { id: number; kind: string; url: string; mimeType: string | null };
type Example = {
    id: number;
    title: string;
...
export default function ShowcaseDetailClient({ slug }: { slug: ShowcaseSlug }) {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const res = await fetch(`/api/showcase/${slug}`, { cache: 'no-store' });
        ...
        <main className="min-h-screen bg-[#020617] text-slate-50 flex flex-col selection:bg-teal-500/30 overflow-hidden relative font-sans">
            ...
            <div className="relative z-10 flex-1">
                <div className="container mx-auto max-w-6xl px-6 pt-28 pb-16">
                    ...
                    {error && (
                        <p className="text-center text-red-400 py-20 font-medium">{error}</p>
                    )}
