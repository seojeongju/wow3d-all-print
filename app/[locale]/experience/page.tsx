...
type SampleMsg = { id: string; name: string; desc: string }
...
export default function ExperiencePage() {
    ...
    const samples = (t.raw('samples') as SampleMsg[]).map((s) => ({
        ...s,
        path: SAMPLE_PATHS[s.id] ?? '/test_cube.stl',
    }))
    ...
    const loadSample = useCallback(
        async (path: string) => {
            ...
            try {
                ...
                if (!res.ok) throw new Error(t('errorLoadSample'))
                const blob = await res.blob()
                const name = path.split('/').pop() || 'sample.stl'
                const f = new File([blob], name, { type: 'model/stl' })
                setFile(f)
            } catch (e) {
                setLoadError(e instanceof Error ? e.message : t('errorLoadFailed'))
            } finally {
    ...
    const [disclaimerBefore, disclaimerAfter] = t('quoteDisclaimer', {
        fullQuote: '\u0000',
    }).split('\u0000')
                                    ...
                                    loadingSample ? (
                                        ...
                                        >
                                            ...
                                            <div className="text-center space-y-2">
                                                <h2 className="text-xl font-black text-white tracking-tight">
                                                    {t('loadingTitle')}{' '}
                                                    <span className="text-teal-400">{t('loadingTitleAccent')}</span>
                                                </h2>
                                        ...
                                        >
                                            ...
                                            <div className="text-center space-y-2">
                                                <h2 className="text-xl font-black text-white tracking-tight">
                                                    {t('analyzingTitle')}{' '}
                                                    <span className="text-indigo-400">{t('analyzingTitleAccent')}</span>
                                                </h2>
                                                <p className="text-sm font-bold text-white/30 tracking-widest uppercase">
                                                    {t('analyzingSubtitle')}
                                                </p>
                                        ...
                                        >
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <p className="text-[11px] font-bold text-white/30 leading-relaxed uppercase tracking-widest px-4">
                                                    {t('errorRetryHint')}
                                                </p>
                                            </div>
                                            <div className="flex flex-col w-full gap-3 px-4">
                                                <Button
                                                    onClick={() => loadSample(samples[0].path)}
                                                    variant="outline"
                                                    className="h-12 rounded-2xl border-white/10 text-white font-black hover:bg-white/5"
                                                >
                                                    {t('retry')}
                                                </Button>
                                                <Link href="/quote" onClick={goBackToSelect} className="w-full">
                                                    <Button className="w-full h-12 rounded-2xl bg-white text-slate-950 font-black hover:bg-white/90 shadow-xl">
                                                        {t('fullQuote')}
                                                    </Button>
                                                </Link>
                                            </div>
                                        ...
                                        >
                                            <div className="space-y-4">
                                                <div className="flex items-center gap-2.5 px-1">
                                                    ...
                                                    <span className="text-[11px] font-black uppercase tracking-[0.3em] text-teal-400/80">
                                                        {t('selectEyebrow')}
                                                    </span>
                                                </div>
                                                <h1 className="text-4xl font-black text-white tracking-tight leading-[1.1]">
                                                    {t('selectTitle')}
                                                    <br />
                                                    <span className="text-teal-400">{t('selectTitleAccent')}</span>
                                                </h1>
                                                <p className="text-[14px] font-bold text-white/40 leading-relaxed break-keep">
                                                    {t('selectSubtitle')}
                                                </p>
                                            </div>

                                            <div className="space-y-5">
                                                <span className="text-[11px] font-black text-white/20 uppercase tracking-[0.25em] px-1">
                                                    {t('samplesLabel')}
                                                </span>
                                                <div className="grid gap-3">
                                                    {samples.map((s) => (
                                                        ...
                                                        >
                                                            ...
                                                            <div className="min-w-0">
                                                                <div className="text-[17px] font-black text-white tracking-tight group-hover:text-teal-400 transition-colors">
                                                                    {s.name}
                                                                </div>
                                                                <div className="text-[12px] font-bold text-white/30 mt-1 leading-tight">
                                                                    {s.desc}
                                                                </div>
                                                            </div>
                                    ...
                                    >
                                        <div className="flex items-center justify-between px-1">
                                            <h2 className="text-2xl font-black text-white tracking-tight">
                                                {t('quoteSettingsTitle')}{' '}
                                                <span className="text-teal-400">{t('quoteSettingsTitleAccent')}</span>
                                            </h2>
                                            <div className="flex items-center gap-3">
                                                ...
                                                <Link
                                                    href="/quote"
                                                    onClick={goBackToSelect}
                                                    className="text-[11px] font-black text-teal-400/80 hover:text-teal-400 tracking-widest uppercase"
                                                >
                                                    {t('fullQuoteLink')}
                                                </Link>
                                            </div>
                                        ...
                                        <div className="pt-8 border-t border-white/5 space-y-6">
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                <Link href="/">
                                                    <Button
                                                        variant="outline"
                                                        className="w-full h-14 rounded-2xl border-white/10 text-white font-black hover:bg-white/5 gap-3 shadow-xl group"
                                                    >
                                                ...
                                                <Link href="/quote" onClick={goBackToSelect}>
                                                    <Button className="w-full h-14 rounded-2xl bg-teal-400 text-slate-950 font-black hover:bg-teal-300 gap-3 shadow-[0_0_30px_rgba(45,212,191,0.3)] transition-all active:scale-[0.98]">
                                                        <Zap className="w-4 h-4 fill-current" /> {t('fullQuoteLink')}
                                                    </Button>
                                                </Link>
                                            </div>
                                        </div>
                        ...
                        <div className="flex-1 relative">
                            ...
                            {!file && !loadingSample && (
                                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none z-10">
                                    ...
                                    <p className="mt-5 text-sm text-white/25 font-medium">{t('viewerEmpty')}</p>
                                </div>
                            )}
                        </div>
