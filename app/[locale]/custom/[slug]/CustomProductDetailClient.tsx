...
export default function CustomProductDetailClient({ product }: { product: CustomProductDef }) {
    ...
    const detailHtml = t(`products.${product.slug}.detailBody`)
    ...
    const optionChoices = (optId: string) => {
        const raw = t(`options.${optId}.choices`)
        return raw
            .split('|')
            .map((s) => s.trim())
            .filter(Boolean)
            ...
            <div className="relative z-10 container mx-auto max-w-6xl px-4 sm:px-6 pt-28 md:pt-36 pb-16 md:pb-24">
                ...
                <div className="grid lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] gap-6 lg:gap-10 items-start mb-10 md:mb-14">
                    ...
                    <div className="rounded-2xl border border-white/10 bg-[#121826]/80 p-5 sm:p-6 md:p-7 space-y-5 lg:sticky lg:top-28">
                        ...
                        <p className="text-sm font-bold text-white/55 leading-relaxed break-keep">{summary}</p>
                        ...
                        <div className="rounded-xl border border-white/10 bg-black/25 px-4 py-3">
                            ...
                            <p className="text-[12px] font-bold text-white/40 mt-1 break-keep">{t('priceHint')}</p>
                        </div>
                        ...
                        <div className="space-y-4 border-t border-white/10 pt-4">
                            ...
                            {product.optionIds.map((optId) => {
                                const choices = optionChoices(optId)
                                return (
                                    <div key={optId} className="space-y-2">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {choices.map((choice) => {
                                                const active = selectedOptions[optId] === choice
                                                return (
                                                    <button
                                                        key={choice}
                                                        type="button"
                ...
                <section className="rounded-2xl border border-white/10 bg-[#121826]/60 overflow-hidden">
                    <div className="border-b border-white/10 px-5 sm:px-8 py-4 flex items-center gap-3">
                        <h2 className="text-lg sm:text-xl font-black text-white">{t('detailSectionTitle')}</h2>
                        <span className="text-[11px] font-bold text-white/35">{t('detailSectionHint')}</span>
                    </div>
                    ...
                    <div className="px-5 sm:px-8 py-8 space-y-8 max-w-3xl mx-auto">
                        <div className="space-y-3 text-center">
                            ...
                            <p className="text-sm font-bold text-white/50 leading-relaxed break-keep">{description}</p>
                        </div>
                        ...
                        <div className="rounded-xl border border-teal-400/25 bg-teal-400/[0.07] p-5 space-y-3 text-center">
                            ...
                            <p className="text-[13px] font-bold text-white/55 break-keep">{t('nextDesc')}</p>
