...
export default function Header() {
    ...
    const navItems = mounted
        ? NAV_ITEMS.map((item) =>
            item.label === t('orders') && user?.role === 'admin'
                ? { ...item, href: '/admin/orders', external: true }
                : item
        )
        : NAV_ITEMS
                            ...
                            {user?.role === 'admin' ? (
                                <NextLink href="/admin">
                                    <button className={`flex items-center justify-center w-9 h-9 sm:w-auto sm:gap-2.5 sm:pl-2 sm:pr-3.5 sm:py-2 rounded-xl transition-all ${
                                        ...
                                        <div className="hidden sm:flex flex-col text-left">
                                            ...
                                            <span className={`text-[10px] leading-tight ${isPastHero ? 'text-slate-400' : 'text-white/50'}`}>Admin</span>
                                        </div>
                                    </button>
                                </NextLink>
                            ) : (
                                <Link href="/my-account">
                                    <button className={`flex items-center justify-center w-9 h-9 sm:w-auto sm:gap-2.5 sm:pl-2 sm:pr-3.5 sm:py-2 rounded-xl transition-all ${
                                        ...
                                        <div className="hidden sm:flex flex-col text-left">
                                            <span className={`text-xs font-bold leading-tight ${isPastHero ? 'text-slate-700' : 'text-white'}`}>{user?.name}</span>
                                            <span className={`text-[10px] leading-tight ${isPastHero ? 'text-slate-400' : 'text-white/50'}`}>Member</span>
                                        </div>
                                    </button>
                                </Link>
                            )}
                ...
                {mobileOpen && (
                    ...
                    >
                        ...
                        >
                            ...
                            <motion.div variants={{ hidden: { x: -20, opacity: 0 }, show: { x: 0, opacity: 1 } }} className="px-1">
